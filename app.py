...
def is_blacklisted(address):
    try:
        ...
        # Get hex address (41xxxx format) and strip leading 41, pad to 64 chars
        addr_hex = val.get("extra", {}).get("address_hex", "")
        if not addr_hex:
            # Fallback: manual base58 decode not needed, use triggerconstantcontract with base58
            addr_hex = ""
        # Remove leading "41" and pad to 64 hex chars
